...
def _generate_stack_inputs(heads, types, prior_order):
	...
	head=heads[child]
	newheads[child]=head
        siblings.append(sibs[head]) #En sibs se guarda un hijo anteriormente creado para ese head, que sera el hermano de child
	#print 'sibs head', sibs[head], head, sibs
        #child_id = child_ids[head]
        skip_connect.append(prev[head])
        prev[head] = position
	...
	children[child-1]=head
        ...
        if newheads[head] != -1 or head == 0:
	    next = 0
	    for i in range(len(newheads)):
		if newheads[i]==-1:
			next=i
			if debug:print 'ROTO11'
			break
	    if next == 0: 
		if debug: 
			print 'ROTO'
		...
		break #significa que ya no quedan nodos por procesar			
            stack=[next]
	else:
            stack.append(head)
        position += 1
