...
class NewStackPtrNet(nn.Module):
	...
	#BERT
	"""	
    	print('Loading BERT from', args.bert_model)
    	self.bert_tokenizer, self.bert = get_bert(args.bert_model, False)
	...
	#position=1
        for t in range(num_step):
	    if debug: print 'ESTAMOS EN EL T=', t, "======================================"
	    ...
	    #BEGIN BEAM
            for id in range(num_hyp * length):
		if debug:
			print id, cc, grand_parents[cc], stacked_heads[cc],  siblings[cc], "_________"
		...
		#print 'ENTRA EN EL BUCLE con nodo ', id
                ...
                base_id = base_index[id]
                child_id = child_index[id]#Coge el hijo mas probable del elemento en head
                head = heads[base_id]
		new_hyp_score = new_hypothesis_scores[id]
		...
		if debug:
			...
			print 'HEADS (child)', child_id, child_index
			print 'CHILD (head)', head, heads
		...
		#new_stacked_heads = [[position] for _ in range(beam)]#Incluimos el proximo nodo a procesar
		new_stacked_heads[cc] = [stacked_heads[base_id][i] for i in range(len(stacked_heads[base_id]))]
